...
void getSmallerAndLarger(std::pair<int, int>& a, std::pair<int, int>& b, std::pair<int, int>& smaller, std::pair<int, int>& larger)
{
...
bool merge(std::pair<int, int>& mergeTarget, std::pair<int, int>& b)
{
...
void mergeAll(std::vector<std::pair<int, int>>& sensorsMinMax)
{
    ...
    while(!isUnmergeable(sensorsMinMax) && !isEverythingMerged(sensorsMinMax))  // isUnmergeable() should go first to not get skipped since it has to update a static variable every time
    {
        ...
        sensorsMinMax = unmerged;
        unmerged.clear();
    }
}

std::vector<std::pair<int, int>> getSensorsMinMaxInRow(std::vector<Sensor> sensors, int row)
{
